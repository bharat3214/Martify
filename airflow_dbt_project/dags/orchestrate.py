...
def orchestrate():
    ...
    @task.bash
    def clean_target():
        return "rm -rf /opt/airflow/retail_analytics_project/target && rm -rf /opt/airflow/retail_analytics_project/logs"

    @task.bash
    def source_freshness():
        return "cd /opt/airflow/retail_analytics_project && dbt source freshness"
    

    silver_technical = BashOperator(
        task_id='silver_technical',
        cwd='/opt/airflow/retail_analytics_project',
